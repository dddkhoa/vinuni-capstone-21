 ...
 */
export const dualSearchTool = ({ session, dataStream }: DualSearchProps) =>
  tool({
    ...
    parameters: z.object({
      query: z
        .string()
        .describe(
          'The search query to find relevant answers from both file database and VinUni policy documents.',
        ),
    }),
    execute: async ({ query }) => {
      try {
        let combinedAnswer = '';
        const combinedCitations: Array<{ title: string; url: string | null }> =
          [];
        ...
        dataStream?.writeData({
          type: 'tool-progress',
          content: {
            step: 'file-search-start',
            message: 'Searching internal file database...',
          },
        });
        ...
        if (vectorStoreId) {
          try {
            ...
            if (
              ...
              fileSearchResults.answer !==
                "I'm sorry, but I can only assist with questions related to VinUni-related topics." &&
              fileSearchResults.answer !==
              ...
              if (
                fileSearchResults.citations &&
                fileSearchResults.citations.length > 0
              ) {
            ...
            dataStream?.writeData({
              ...
              content: {
                ...
                message:
                  'File search encountered an error, continuing with Tavily search...',
              },
            });
          }
        } else {
          dataStream?.writeData({
            ...
            content: {
              ...
              message:
                'File search unavailable (missing VECTOR_STORE_ID), proceeding to Tavily search...',
            },
          });
        }
        ...
        dataStream?.writeData({
          type: 'tool-progress',
          content: {
            step: 'tavily-search-start',
            message: 'Searching VinUni policy documents...',
          },
        });
        ...
        if (process.env.TAVILY_API_KEY) {
          try {
            ...
            if (
              ...
              if (
                tavilyResults.citations &&
                tavilyResults.citations.length > 0
              ) {
              ...
              dataStream?.writeData({
                ...
                content: {
                  step: 'tavily-search-complete',
                  message:
                    'No relevant results found in VinUni policy documents',
                  resultsFound: false,
                },
              });
            }
        ...
        if (!hasFileResults && !hasTavilyResults) {
          return {
            answer:
              "I couldn't find relevant information in either the file database or VinUni policy documents. Please try rephrasing your question or asking about other VinUni-related topics.",
            citations: [],
            searchStats: {
              fileSearchCompleted: !!vectorStoreId,
              tavilySearchCompleted: !!process.env.TAVILY_API_KEY,
              totalSources: 0,
            },
          };
        }
