...
export const fetchFileSearch = async ({
  ...
  try {
    ...
    const { textAnswer, annotations } = await performVectorSearch(
      query,
      vectorStoreId,
    );
    ...
    if (textAnswer === 'NOT_FOUND') {
      const { textAnswer: webAnswer, annotations: webAnnotations } =
    ...
    const filenames = extractUniqueFilenames(
      annotations as ExtendedFileCitation[],
...
export const fileSearchTool = tool({
  ...
  parameters: z.object({
    query: z
      .string()
      .describe('The search query to find relevant answers from files.'),
  }),
  execute: async ({ query }) => {
    try {
